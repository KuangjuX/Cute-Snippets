...
from htile import VectorCopy, row_reduce
    ...
    def kernel(
        ...
        tXpX = (
            ...
            if is_even_N
                ...
                thr_copy_X.partition_S(cX), limit=shape[1]
            )
        )
        ...
        exp_x = cute.math.exp2(x * log2_e - (max_x * log2_e), fast_math=True)
        denom = row_reduce(
            exp_x,
            cute.eductionOp.ADD,
            threads_per_row,
...
def test_softmax():
    ...
    try:
        ...
        # Check results
        out_cpu = out.cpu()
        ref = torch.softmax(x.cpu().to(torch.float32), dim=1).to(torch.float16)
        max_diff = torch.max(torch.abs(ref - out_cpu)).item()
        print(f"Output (first 4): {out_cpu.flatten()[:4]}")
        print(f"Softmax ref (first 4): {ref.flatten()[:4]}")
